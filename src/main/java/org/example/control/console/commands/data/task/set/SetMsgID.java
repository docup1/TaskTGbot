...
public class SetMsgID extends Command {
    @Override
    public void run(ArrayList<String> args) {
        if (args.size() < 2) {
            setResponse(new Response(Status.BAD_REQUEST, ContentType.TEXT_PLAIN, "Insufficient arguments. Expected message_id and task_id."));
            return;
        }

        String query = "UPDATE Tasks SET msg_id = ? WHERE task_id = ?;";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, args.get(1)); // message_id
            statement.setInt(2, Integer.parseInt(args.get(0))); // task_id

            int rowsAffected = statement.executeUpdate();
            String message = rowsAffected > 0
                    ? "Task message ID updated successfully. Rows affected: " + rowsAffected
            ...
            setResponse(new Response(Status.BAD_REQUEST, ContentType.TEXT_PLAIN, "Error: Both task ID and message ID must be valid integers."));
        }
    }
}
